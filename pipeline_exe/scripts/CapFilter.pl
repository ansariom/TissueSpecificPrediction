...
my ($command, $results, $fasta, $seq_file, $gzip, $gzip_out, $output_file, $bam_file, $genome_file, $peak_file, $bed, $peak_cutoff, $peak_header_line, @peak_ref_order, $sorted, $help);
...
sub parse_genome {
	...
	while (my $line = <DAT>) {
		...
		# $seq .= $line if eof(DAT); SHAWN FIX BUG HERE
		if(eof(DAT)) {
			$seq .= $line;
			$genome{$id} = $seq;
		}
