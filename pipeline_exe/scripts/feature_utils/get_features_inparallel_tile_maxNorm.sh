...
for seqfile in `ls $seqs_outdir/$basename.*`; do
	...
	java -Xms10G -Xmx20G -jar external_tools/tfbs_scan.jar GenFeaturesTiledNormByMax --nucsUp $tile_nucsUp --nucsDown $tile_nucsDown --winWidth $tileWin --mapFile $maps_out -n 5 $seqfile $pwm $outfile --maxScores $maxScoresFile  &	
