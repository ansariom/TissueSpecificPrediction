...
all_tss_features = args[2]
model_type = args[3]
features_outfile <- args[4]
diff_info_outfile <- args[5]
fc <- as.numeric(args[6])

# into all_features_diffs_wide
load(input_features)
# all_tss_diffs_wide
load(all_tss_features)

if (model_type == "roe_only") {
  ### or, remove tiled features
  all_tss_diffs_wide <- all_tss_diffs_wide[, !grepl("tile", colnames(all_tss_diffs_wide)) ]
}

features <- all_tss_diffs_wide[, colnames(all_features_diffs_wide)]
print(features[1:3, 1:30])
# set rownames to tss names
rownames(features) <- features$tss_name


# define classes, get rid of unclassed columns
classed_features_diffs_wide <- add_class(features, qval_thresh = 0.05, fold_thres = fc)
