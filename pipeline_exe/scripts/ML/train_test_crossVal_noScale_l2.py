...
batch_size = 100
display_step = 50
weight_decays = np.arange(0.0001, max_param ,0.02) 
fold = 0
fold_results = []
outdir = "-"
...
#        scaler = preprocessing.StandardScaler(with_mean=True, with_std=True).fit(train_fold_features)
	...
	scaled_train_features = train_fold_features
	scaled_test_features = test_fold_features
...
#     print(mean_param)
    ...
    for key, group in grouped:
        group.plot(ax=ax, kind='line', x='param', y='auc', label=key, color=colors[key], xlim = [0, 0.2], ylim = [0.4, 1])
...
if __name__ == "__main__":
    ...
    seeds = np.random.randint(1200000, size = 10)
#    seeds = [578510, 388638]
#    seeds = [107244] 
