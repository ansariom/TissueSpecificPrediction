...
print(tfbs_features[1:10,1:10])

tfbs_features <- as.data.frame(apply(tfbs_features, MARGIN = 2, function(x) {x/max(x)}))
print(tfbs_features[1:10,1:10])

if (model_type == "roe_only" ) {
	oc_features <- classed_features_class[, grepl("_OC_", colnames(classed_features_class))]
	classed_features_class <- cbind(oc_features, tfbs_features)
} else {
	classed_features_class <- tfbs_features
}
