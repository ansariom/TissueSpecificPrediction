...
## normalize TFBS values between 0-1 and don't change OC features
tfbs_features <- classed_features_class[, !grepl("_OC_", colnames(classed_features_class))]
tfbs_features <- tfbs_features[, !colnames(tfbs_features) %in% "class"]

tfbs_features <- as.data.frame(apply(tfbs_features, MARGIN = 2, function(x) {x/sd(x)}))

oc_features <- classed_features_class[, !colnames(classed_features_class) %in% colnames(tfbs_features)]

classed_features_class <- cbind(tfbs_features, oc_features)
