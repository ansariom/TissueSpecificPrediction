...
batch_size = 100
display_step = 50
weight_decays = np.arange(0.00001, max_param ,0.0003) 
fold = 0
fold_results = []
outdir = "-"
...
#        scaler = preprocessing.MinMaxScaler().fit(train_fold_features)
        scaler = preprocessing.StandardScaler(with_mean=False, with_std=True).fit(train_fold_features)
...
if __name__ == "__main__":
	...
	#mean_param = 0.000266
    	...
    	scaler = preprocessing.StandardScaler(with_mean=False, with_std=True).fit(train_all_features)
