...
batch_size = 100
display_step = 50
weight_decays = np.arange(0.00001, max_param ,0.001) 
fold = 0
fold_results = []
outdir = "-"
...
#        scaler = preprocessing.MinMaxScaler().fit(train_fold_features)
        scaler_std = preprocessing.StandardScaler(with_mean=False, with_std=True).fit(train_fold_features)
	train_sd_scaled = scaler_std.transform(train_fold_features)

	scaler_minmax =  preprocessing.MinMaxScaler().fit(train_sd_scaled)
        scaled_train_features = scaler_minmax.transform(train_sd_scaled)

	test_sd_scaled = scaler_std.transform(test_fold_features)
        scaled_test_features = scaler_minmax.transform(test_sd_scaled)
...
if __name__ == "__main__":
	...
	#mean_param = 0.000266
    	...
    	scaler_std = preprocessing.StandardScaler(with_mean=False, with_std=True).fit(train_all_features)
	train_std_scaled = scaler_std.transform(train_all_features)
	test_std_scaled = scaler_std.transform(test_all_features)

        scaler_minmax = preprocessing.MinMaxScaler().fit(train_std_scaled)
	scaled_train_all = scaler_minmax.transform(train_std_scaled)
	scaled_test_all = scaler_minmax.transform(test_std_scaled)	
